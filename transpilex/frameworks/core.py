...
def format_cshtml_files(directory):
    for cshtml_file in Path(directory).rglob("*.cshtml"):
        print(f"Formatting {cshtml_file}...")
        try:
            # Use '--include' to specify a single file
            result = subprocess.run(
                ["dotnet", "format", "--include", str(cshtml_file)],
                capture_output=True,
                text=True,
            ...
            print(result.stdout)
            if result.stderr:
                print(f"Error (if any) during format: {result.stderr}")
            print(f"Formatted: {cshtml_file.name}")
        except subprocess.CalledProcessError as e:
            print(f"Failed to format {cshtml_file.name}: {e}")
...
def create_core_project(project_name, source_folder, assets_folder):
    ...
    format_cshtml_files(pages_path)
