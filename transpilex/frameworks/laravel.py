...
def extract_params_from_include(include_string):
    ...
    print(f"Warning: Could not extract parameters from include string: {include_string}")
    return {}


def format_blade_include_params(data_dict):
    """
...
def format_page_title_blade_include(data_dict):
    """
    Generates the full Blade @include directive for page-title with all parameters.
    """
    formatted_params = format_blade_include_params(data_dict)
...
def convert_to_laravel(dist_folder):
    ...
    for file in dist_path.rglob("*"):
        if file.is_file() and file.suffix == ".php":
            ...
            original_content = content  # Keep original content for comparison and fallback
            ...
            layout_title = ""
            if title_meta_match:
                meta_data = extract_params_from_include(title_meta_match.group())
            ...
            for script_tag in scripts_html:
                ...
                if src_attr:
                    ...
                    # This turns "/assets/js/main.js", "./assets/js/main.js", "../assets/js/main.js"
                    # into "assets/js/main.js"
                    normalized_src_attr = re.sub(r'^(?:(?:\.\/|\.\.\/)*\/)*', '', src_attr)

                    # Condition 1: Path starts with 'assets/js/' and is not excluded
                    ...
                    # Condition 2: Path starts with 'js/' (but not 'assets/js/') and is not excluded
            ...
            print(f"ℹ️ For '{file.name}': Content for @section('content') taken from {content_source_info}.")
            ...
            if page_title_match:
                page_data = extract_params_from_include(page_title_match.group())
                blade_include = format_page_title_blade_include(page_data)
                inner_html = re.sub(
                    r'@@include\(\s*["\']\.\/partials\/page-title(?:\.html)?["\']\s*,\s*(array\(.*?\)|{.*?})\s*\)',
                    blade_include,
                    inner_html,
            ...
            # --- Final Blade structure assembly ---
...
"""
            ...
            # Clean relative asset paths in the final output
